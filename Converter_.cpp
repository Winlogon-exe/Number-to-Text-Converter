...
std::string rubleEnding(int number) {
    //берем 1 число с конца 
    int lastDigit = number % 10;
    //берем 2 число с конца
...
std::string nameForNumber(long long number) {
    ...
    //десятки
    else if (number < 100) return tens[number / 10] + nameForNumber(number % 10);
    //сотни
    else if (number < 1000) return hundredth[number / 100] + nameForNumber(number % 100);
    //тысячи
    else if (number < 1000000) return nameForNumber(number / 1000) + thousandEnding(number / 1000) + nameForNumber(number % 1000);
    //миллионы
...
int main()
{
    ...
    do
    {
        ...
        cin >> input;
        cout << "\n" << nameForNumber(input) + rubleEnding(input) << "\n\n";
    } while (input > 0);
    return 0;
}
